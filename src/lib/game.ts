...
import { Grid, BOARD_HEIGHT, BOARD_BORDER_HEIGHT, CellValue, isLineComplete } from './board';
    ...
    for (let c = 0; c < shape[r].length; c++) {
      if (shape[r][c] !== 0) {
        ...
        if (boardY >= 0 && boardY <= 21 && boardX >= 0 && boardX < 10) {
          ...
          const colorValue = TETROMINOES[state.currentPiece.type].color;
          const COLOR_MAP: Record<string, number> = { '#00FFFF': 1, '#FFFF00': 2, '#0000FF': 3, '#FF7F00': 4, '#00FF00': 5, '#FFFFFF': 6, '#800080': 7 };
          board[boardY][boardX] = COLOR_MAP[colorValue];
        }
      }
    }
  }
...
// Try to move piece left (with wall kicks)
}
export function tryMoveLeft(board: Grid, state: GameState): boolean {
    if (!state.currentPiece) return false;
  const shape = getPieceShape(state.currentPiece);
  
  // Check if current position allows movement
  for (let r = 0; r < shape.length; r++) {
    for (let c = 0; c < shape[r].length; c++) {
      if (shape[r][c] !== 0) {
        const boardY = state.currentPiece.y + r;
        const boardX = state.currentPiece.x + c - 1;
        
        // Check boundaries
        if (boardX < 0 || boardY < 0 || boardY > 20 || board[boardY][boardX] !== 0) {
          return false; // Blocked by wall or piece
        }
      }
    }
  }
...
// Try to move piece right (with wall kicks)
export function tryMoveRight(board: Grid, state: GameState): boolean {
    if (!state.currentPiece) return false;
  const shape = getPieceShape(state.currentPiece);
  ...
  for (let r = 0; r < shape.length; r++) {
    for (let c = 0; c < shape[r].length; c++) {
      if (shape[r][c] !== 0) {
        ...
        // Check boundaries
        if (boardX >= 10 || board[boardY][boardX] !== 0) {
          return false; // Blocked by wall or piece
        }
      }
    }
